...
{	
	...
	MixStreamer* streamer = (__bridge MixStreamer *)inClientData;
	[streamer
...
{
	MixStreamer* streamer = (__bridge MixStreamer *)inUserData;
...
{
	MixStreamer* streamer = (__bridge MixStreamer *)inClientInfo;
	[streamer handleReadFromStream:aStream eventType:eventType];
}

@implementation MixStreamer
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		if (err)
		{
			...
			NSLog(@"%@ err: %c%c%c%c %d\n",
				[MixStreamer stringForErrorCode:anErrorCode],
				errChars[3], errChars[2], errChars[1], errChars[0],
				(int)err);
		}
		else
		{
			NSLog(@"%@", [MixStreamer stringForErrorCode:anErrorCode]);
		}
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			AudioFileTypeID fileTypeHint =
				[MixStreamer hintForFileExtension:self.fileExtension];
